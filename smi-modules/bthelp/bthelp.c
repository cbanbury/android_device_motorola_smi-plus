...
static const char *brfChip;
static const char *c_brf_chip[16] = {
	"unknown",
	"unknown",
	"unknown",
	"unknown",
...
static void getBrfChip(const char *lmp)
{
	unsigned short lmp_subv = lmp[13] | (lmp[14] << 8);
	unsigned short brf_chip = (lmp_subv & 0x7e00) >> 9;

	if(brf_chip >= sizeof(c_brf_chip))
		brfChip = NULL;
	else
		brfChip = c_brf_chip[brf_chip];
};
			     ...
			     char *firmware, char *sleepmode)
{
	...
	if(brfChip != NULL && strcmp(brfChip, "unknown")) {
		char fw[100];
		sprintf(fw, "/etc/firmware/%s.bin", brfChip);
		err = texas_load_firmware(fd, fw, sleepmode);
	 } else
